...
class Trainer:
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            features = self.models["encoder"](inputs["color_aug", 0, 0])
            # outputs = self.models["depth"](features)
            outputs = self.models["depth"](features, pre_disp[('disp', 0)])
            for scale in self.opt.scales:
        ...
        l1_loss=l1(pre_disp['disp', 0], outputs["disp",0])

        losses['l1']=l1_loss
        # losses['l2']=l2_loss
        losses['photomatric']=losses["loss"]

        losses['ddim']=outputs["ddim_loss"]
    ...
    def process_batch_mask(self, inputs, outputs):
        ...
        """  
        features = self.models["encoder"](inputs["color_aug", 0, 0])   
        outputs_mask = self.models["depth"](features, outputs[('predisp', 0)], outputs["identity_selection/{}".format(0)].unsqueeze(1))
        mask_losses = 0
        for scale in self.opt.scales:
